...
public class AdminMovieDAO {
		...
		DbConnection dc = DbConnection.getInstance();
		  ...
		  try {
		      ...
		      //4. 쿼리문 생성객체 얻기
		    	...
		    	selectQuery
		    	.append("	select mv_no,mv_title,mv_poster,mv_director,mv_st,mv_trailer,mv_opendate,mv_closedate,mv_runtime,mv_regdate,mv_openornot,mv_genre,mv_soon_porster	")
		    	.append("	from movie	")
		      ...
		      //6. 쿼리문 수행.
		    	rs=pstmt.executeQuery();
		    	if(rs.next()) {		    		
		    		amsVO=new AdminMovieSelectVO(rs.getString("mv_no"),rs.getString("mv_poster"),rs.getString("mv_soon_porster"),rs.getString("mv_title"),rs.getString("mv_director"),selectActor(rs.getString("mv_no")),rs.getString("mv_st"),rs.getString("mv_runtime"),rs.getString("mv_trailer"),rs.getString("mv_opendate"),rs.getString("mv_openornot"),rs.getString("mv_genre"));
		    		list.add(amsVO);
		    	}
		      }finally {
		      //7. 연결끊기
	...
	public void insertMovie(MovieUpdateInsertVO muiVO) throws SQLException {
		...
		try {
			con=dc.getConn();
			//mv_no에 concat('m_',lpad(시원스이름.nextval,8,0))
			String insertQuery="insert into movie(mv_no,mv_title,mv_poster,mv_director,mv_st,mv_attcnt,mv_trailer,mv_opendate,mv_closedate,mv_runtime,mv_regdate,mv_openornot,mv_genre,mv_soon_porster) values(?,?,?,?,?,0,?,?,sysdate,?,sysdate,?,?,?)";
	...
	public void insertActorArr(MovieUpdateInsertVO muiVO) throws SQLException {
		
		ActorVO acVO=new ActorVO();
		
		Connection con=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		
		String[] actorArr=muiVO.getActorNameArr();
		String[] roleArr=muiVO.getActorRoleArr();
		...
		DbConnection dc = DbConnection.getInstance();
		  ...
		  try {
		      ...
		      //4. 쿼리문 생성객체 얻기
		    	...
		    	String insertQuery="insert into actor(mv_no,act_name,act_mainorsub) values('"+muiVO.getMvNo()+"',?,?)";
	    				    			
		    	pstmt=con.prepareStatement(insertQuery);
		      //5. 바인드 변수에 값 설정.
		    	 
		    	 pstmt.setString(1,actorArr[i]); 
		    	 pstmt.setString(2,roleArr[i]); 
		      //6. 쿼리문 수행.
		    	rs=pstmt.executeQuery();
		    	}
		      }finally {
